...
static const std::string CSV_HEADER_EN = "Round,Total,Success,Error,Avg(ms),Max(ms),Min(ms),TotalTime(s),Throughput(tps),SuccessRate,ErrorRate";
static const std::string CSV_HEADER_ZH = "轮次编号,总执行次数,成功次数,失败次数,平均耗时(ms),最大耗时(ms),最小耗时(ms),总耗时(s),吞吐量(tps),成功率,错误率"; 
...
int main(int argc, char* argv[])
{
    ...
    for (size_t r = 0; r < ROUNDS; ++r)
    {
        ...
        {
            scheduler->add_once_job(std::chrono::system_clock::now() + std::chrono::milliseconds(10), [&, i]() {
        ...
        double avg_ms = durations.empty() ? 0 : total_duration.count() * 1.0 / durations.size();
        ...
        out << r + 1 << ","
            ...
            << avg_ms << ","
            << max_duration.count() << ","
            << (min_duration == std::chrono::milliseconds::max() ? 0 : min_duration.count()) << ","
            << r_duration << ","
            << tps << ","
            << succ_rate << ","
            << err_rate << "\r\n";
        ...
        out.flush();
        std::cout << "[Round " << r + 1 << "] 一次性任务压测完成 ✅" << std::endl;
    }
