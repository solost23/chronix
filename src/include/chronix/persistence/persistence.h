...
public:
    ...
    std::vector<T> load() override
    {
        ...
        if (j.is_discarded())
        {
            std::cerr << "[Error] JSON parse failed when loading: " << filepath
                      << std::endl;
            return {};
        }
...
{
public:
    DBPersistenceMySQL(const std::string& host, int port,
                       const std::string& user, const std::string& password,
                       const std::string& schema)
        : session(host, port, user, password), db(session.getSchema(schema))
    {}
    ...
    void save(const std::vector<T>& jobs) override
    {
        ...
        {
            session.rollback();
            std::cerr << "Error during save operation: " << e.what()
                      << std::endl;
        }
    }
